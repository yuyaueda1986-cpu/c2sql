...
> **更新（2026-06）**: 旧版で非ゴールだった「SQLite3以外の同梱ドライバ」は、
> PostgreSQL（libpq）ドライバとして提供済み（`-DC2SQL_WITH_POSTGRES=ON` で有効化、
> 既定はSQLiteのみ）。詳細は後述「マルチバックエンドとSQL方言」節。
...
ドライバ抽象（vtable）に `dialect` フィールド（`C2SQL_DIALECT_SQLITE` /
`C2SQL_DIALECT_POSTGRES`）を持たせ、Query Builder が方言別に SQL を生成する。

| 項目 | SQLite | PostgreSQL |
|--|--|--|
| プレースホルダ | `?` | `$1, $2, …` |
| INT32 / INT64 | `INTEGER` / `INTEGER` | `INTEGER` / `BIGINT` |
| REAL | `REAL` | `DOUBLE PRECISION` |
| BLOB | `BLOB` | `BYTEA` |
| 厳格型 | `) STRICT` 付与 | 不要（既定で静的型） |
| UPSERT | `ON CONFLICT(pk) DO UPDATE SET col=excluded.col`（両方言で共通） | 同左 |

### ドライバ選択

`SqlRDBInit(dsn)` は接続文字列のスキームでドライバを選ぶ:

- `postgresql://…` / `postgres://…` → PostgreSQL ドライバ（`HAVE_POSTGRES` ビルド時）
- それ以外 → SQLite（ファイルパス / `file:` URI / `:memory:`）

PostgreSQL 無効ビルドで PostgreSQL DSN を渡すと `SqlRDBInit` は `NULL` を返す。
公開API・構造体・エラーコードはバックエンド非依存で不変。

### libpq ドライバの実装要点

- SQLite は「bind→step」逐次型、libpq は「全パラメータ一括 `PQexecParams`」型。
  ドライバはバインド値を蓄積し、最初の `step()` で遅延実行、以降は結果行を走査する。
  `reset()` で結果と束縛をクリアし、ステートメントキャッシュで再利用できる。
- パラメータは text 形式で送り、サーバが文脈から型推論（BLOB のみ bytea OID 指定の binary）。
- 結果は text 形式で受け、BLOB 列は `PQunescapeBytea` で復号。
- スキーマ突合・自動マイグレーションは `information_schema.columns` と `pg_index`
  による introspection で実装（SQLite の `PRAGMA` 経路と対をなす）。STRICT 判定はSQLite専用。
