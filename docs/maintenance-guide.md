...
libc2sqlの内部はドライバ抽象化されており、PostgreSQL（libpq）ドライバを提供する
（`-DC2SQL_WITH_POSTGRES=ON` でビルド、既定はSQLiteのみ）。利用者APIは不変で、
接続文字列のスキーム（`postgresql://` / `postgres://`）でバックエンドが切り替わる。

PostgreSQL側のスキーマ突合・自動マイグレーションは `information_schema.columns` と
`pg_index` による introspection で行う（SQLiteの `PRAGMA` に相当）。`STRICT` は
SQLite固有のため、PostgreSQLでは常に「厳格」とみなす。型は次のように対応する:

| `SqlRDBType` | SQLite | PostgreSQL |
|--|--|--|
| `SQL_TYPE_INT32` | `INTEGER` | `INTEGER` |
| `SQL_TYPE_INT64` | `INTEGER` | `BIGINT` |
| `SQL_TYPE_REAL` | `REAL` | `DOUBLE PRECISION` |
| `SQL_TYPE_TEXT` | `TEXT` | `TEXT` |
| `SQL_TYPE_BLOB` | `BLOB` | `BYTEA` |

検査はそれぞれのCLI（`sqlite3` / `psql`）で通常どおり行える。
